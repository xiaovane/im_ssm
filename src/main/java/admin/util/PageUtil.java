...
public class PageUtil {
    ...
    public static String getPrevAndNextPageCode(WebUser prev, WebUser next, String projectContent) {
        StringBuffer pageCode = new StringBuffer();
        if(prev == null || prev.getId() == null) {
            pageCode.append("<p>上一篇：无</P>");
        } else {
            pageCode.append("<p>上一篇：<a href='" + projectContent + "/WebUser/articles/" + prev.getId() + ".html'>" + prev.getTitle() + "</a></p>");
        }

        if(next == null || next.getId() == null) {
            pageCode.append("<p>下一篇：无</P>");
        } else {
            pageCode.append("<p>上一篇：<a href='" + projectContent + "/WebUser/articles/" + next.getId() + ".html'>" + next.getTitle() + "</a></p>");
        }
    ...
    public static String getUpAndDownPageCode (
        ...
        if(totalPage == 0) {
            ...
            if(page > 1) {
                pageCode.append("<li><a href='"+projectContext+"/WebUser/search.html?page="+(page-1)+"&q="+q+"'>上一页</a></li>");
            } else {
                pageCode.append("<li class='disabled'><a>上一页</a></li>");
            }
            if(page < totalPage) {
                pageCode.append("<li><a href='"+projectContext+"/WebUser/search.html?page="+(page+1)+"&q="+q+"'>下一页</a></li>");
            } else {
                pageCode.append("<li class='disabled'><a>下一页</a></li>");
            }
            pageCode.append("</ul>");
