...
public class WebUserIndex {
     ...
     * @throws IOException
     */
    public void addIndex(WebUser WebUser) throws IOException {
        //获取写索引对象
        IndexWriter indexWriter = getIndexWriter();
        //实例化索引文档
        Document document = new Document();
        //设置索引文件字段
        document.add(new StringField("id",String.valueOf(WebUser.getId()), Field.Store.YES));
        document.add(new TextField("title",WebUser.getTitle(), Field.Store.YES));
        document.add(new StringField("releaseDate", DateUtil.formatDate(new Date(),"yyyy-mm-dd"), Field.Store.YES));
        document.add(new TextField("content",WebUser.getContentNoTag(), Field.Store.YES));
        //索引文档加入到写索引中
     ...
     * @param WebUserId
     */
    public void deleteIndex(String WebUserId) throws IOException {
        IndexWriter indexWriter = getIndexWriter();
        indexWriter.deleteDocuments(new Term("id",WebUserId));
     ...
     * @throws IOException
     */
    public void updateIndex(WebUser WebUser) throws IOException {
        //获取写索引对象
        IndexWriter indexWriter = getIndexWriter();
        //实例化索引文档
        Document document = new Document();
        ...
        document.add(new TextField("content",WebUser.getContentNoTag(), Field.Store.YES));
        //更新索引文档
        indexWriter.updateDocument(new Term("id",String.valueOf(WebUser.getId())),document);
        //关闭写索引
        indexWriter.close();
    }

    public List<WebUser> searchWebUser(String q) throws Exception{
        //实例化目录对象
        directory = FSDirectory.open(Paths.get(indexDir));
        //获取读索引对象
        ...
        //用来封装查询到
        List<WebUser> WebUserIndexList = new LinkedList<WebUser>();
        //遍历查询结果
        for(ScoreDoc score : hits.scoreDocs) {
            Document doc = indexSearcher.doc(score.doc);
            WebUser WebUser = new WebUser();
            WebUser.setId(Integer.parseInt(doc.get("id")));
            WebUser.setReleaseDateStr(doc.get("releaseDate"));
            ...
            if(title != null) {
                ...
                if(StringUtil.isEmpty(hTitle)) {
                    WebUser.setTitle(title);
                } else {
                    WebUser.setTitle(hTitle);
                }
            }
            if(content != null) {
                ...
                if(StringUtil.isEmpty(hContent)) {
                    if(content.length() > 100) { //如果没查到且content内容又大于100的话
                        WebUser.setContent(content.substring(0, 100)); //截取100个字符
                    } else {
                        WebUser.setContent(content);
                    }
                } else {
                    WebUser.setContent(hContent);
                }
            }
            WebUserIndexList.add(WebUser);
        }

        return WebUserIndexList;
    }
}
