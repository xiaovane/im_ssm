...
(function(globalContext, factory) {
    ...
    // 不重复执行
    var moduleExports = globalContext.ejs || factory(globalContext);

    if (typeof module !== 'undefined' && module.exports) {
        // 用exports，导出一个MiniRefreshTools对象
        exports.ejs = moduleExports;
    ...
    // 单独引入时暴露的是这个tools 
    globalContext.ejs = moduleExports;
            ...
            if (ejs.os.ejs) {
                ...
                ejs.event.config(ejs.innerUtil.extend({
                    ...
                    error: function(error) {
                        var tips = error ? JSON.stringify(error) : ejs.globalError.ERROR_TYPE_CONFIGERROR.msg;
                        
                        errorTips(ejs.globalError.ERROR_TYPE_CONFIGERROR.code, tips);
                    }
                }, params));
...
(function(exports) {
    ...
    var innerUtil = {
        ...
        /**
         * each遍历操作
         * @param {type} elements 需要被变量的元素
         * @param {Function} callback 回调
         * @param {Function} hasOwnProperty 过滤函数
         * @returns {global} 返回上下文对象
         */
        ...
        extend: function() {
            ...
            for (; index < len; index++) {
                ...
                if (source && innerUtil.isObject(source)) {
         ...
         * @param {Array} osArray 系统组合
         * @param {String} name 需要判断的系统名
         ...
         */
        eclipseText: function(str, count) {
            var res = '',
                num = 0,
                LEN_CHINESE = 2,
                LEN_ENGLISH = 1;
...
(function(exports) {
     ...
     * @return {Function} 返回处理后的回调，也就是最终的执行函数
     */
...
(function(exports) {
     ...
     * @param {Array} namespaceArr 命名空间数组
    ...
    function extendApi(moduleName, api) {
        ...
        for (var i = 0, len2 = osArray.length; i < len2; i++) {
            tmp = osArray[i];
        ...
        if (oldNameSpace.api && oldNameSpace.api.os && api.os) {
            ...
            for (var j = 0, len3 = oldNameSpace.api.os.length; j < len3; j++) {
                if (api.os.indexOf(oldNameSpace.api.os[j]) === -1) {
                    api.os.push(oldNameSpace.api.os[j]);
                }
            }
        }
        // 如果老的代理已经不用了
