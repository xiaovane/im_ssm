...
<head>
    ...
    <script>
        ...
        function formatWebUserTile(val,row) {
            if(val.id == null){
                return "<font color=red>该已删除</font>";
            }else{
                return "<a href='${WebUser}/WebUser/articles/"+val.id+".html' target='_blank'>"+val.title+"</a>";
            }
        }
        /**
         * 审核评论
        ...
        function commentReview(state) {
            ...
            $.messager.confirm("系统提示", "<font color=red>您确定要审核选中的"+selectedRows.length+"条数据么？</font>", function(r) {
                if(r) {
                    $.post("${WebUser}/admin/comment/review.do",
...
<table id="dg" title="评论信息管理" class="easyui-datagrid" fitColumns="true" pagination="true"
       url="${WebUser}/admin/comment/list.do?state=0" toolbar="#tb">
    <thead>
    <tr>
        <th field="cb" checkbox="true" align="center"></th>
        <th field="id" align="center" width="20px">编号</th>
        <th field="userIp" align="center" width=100px">评论者ip</th>
        <th field="WebUser" align="center" width="150px" formatter="formatWebUserTile">标题</th>
