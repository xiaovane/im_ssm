	...
	function submitData() {
		...
		if(content == null || content == "") {
			...
			$.post(
				...
				{"content":content,"imageCode":imageCode,"WebUser.id":"${WebUser.id}"},
				function(result) {
					if(result.success) {
...
<div class="data_list">
	<div class="data_list_title">
		<img
			src="${pageContext.request.contextPath}/static/images/WebUser_show_icon.png" />&nbsp;信息
	</div>
	<div>
		...
		<div class="WebUser_info">
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach items="${keyWords }" var="keyword">
							&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/WebUser/search.html?q=${keyword }">${keyword }</a>&nbsp;
						</c:forEach>
					</c:otherwise>
				</c:choose>
			</div>
			<div style="float:right;">
				发布于：『
				<fmt:formatDate value="${WebUser.releaseDate }" type="date"
					pattern="yyyy-MM-dd HH:mm" />
				』&nbsp;&nbsp; 类别：<a
					href="${pageContext.request.contextPath}/index.html?typeId=${WebUser.InfoType.id }">${WebUser.InfoType.typeName }</a>&nbsp;&nbsp;
		...
		<div class="WebUser_content">${WebUser.content }</div>
